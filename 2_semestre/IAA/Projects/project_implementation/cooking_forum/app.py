...
def index():
  ...
  response = requests.get('http://cooking_resource_server:8022/get_all_posts', headers=headers)
...
def create():
  if request.method == 'POST':
    ...
    headers = {'Authorization': f'{access_token}'}
    response = requests.post('http://cooking_resource_server:8022/create_post', json=data, headers=headers)
